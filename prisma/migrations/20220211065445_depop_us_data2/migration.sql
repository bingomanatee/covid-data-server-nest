/*
  ...
  - You are about to drop the column `population` on the `covid_daily_cases_usa` table. All the data in the column will be lost.
  ...
  - A unique constraint covering the columns `[uid]` on the table `locations_usa` will be added. If there are existing duplicate values, this will fail.
...
-- DropIndex
DROP INDEX "covid_daily_cases_usa_date_published_idx";

-- DropIndex
DROP INDEX "covid_daily_cases_usa_uid_idx";
...
-- DropIndex
DROP INDEX "locations_usa_uid_key";

-- DropIndex
DROP INDEX "source_files_path_key";

-- AlterTable
ALTER TABLE "covid_daily_cases_usa" DROP COLUMN "population";

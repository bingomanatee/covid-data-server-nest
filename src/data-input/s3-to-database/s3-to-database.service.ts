...
export class S3ToDatabaseService {
  ...
  public async writeS3FileRows(newS3File: any) {
    ...
    inputStream
      ...
      .on('data', function (row) {
        ...
        target.loggingService.info('writeS3FileRows --- writing row %d, %s', rowCount, 
        JSON.stringify(row)
        );
        }
      })
      .on('error', (err) => {
        target.loggingService.error('writeS3FileRows --- parse error: %s', err.message);
  ...
  private async noteStartTime(startDate: Date, newS3File) {
    ...
    this.loggingService.info('got s3Info for %s, = %s'
    , newS3File.path, JSON.stringify(s3Info));
